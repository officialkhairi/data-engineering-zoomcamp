...
with
    unioned as (select * from {{ ref("int_trips_unioned") }}),
    ...
    cleaned_and_enriched as (
        ...
        from unioned u
        left join payment_types pt on coalesce(u.payment_type, 0) = pt.payment_type
    )

select *
from cleaned_and_enriched

-- Deduplicate: if multiple trips match (same vendor, second, location, service), keep
-- first
qualify
    row_number() over (
